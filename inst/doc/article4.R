...
mod <- bass(x, y, nmcmc = 40000, nburn = 30000, thin = 10,
            temp.ladder = (1 + .35)^(1:9 - 1), verbose = F)
...
mod <- bass(x, y, nmcmc = 40000, nburn = 30000, thin = 10,
            temp.ladder = (1 + .25)^(1:9 - 1), verbose = F)
n.test <- 1000
...
q1 <- apply(pred.error, 2, quantile, probs = .05)
q2 <- apply(pred.error, 2, quantile, probs = .95)
...
mod <- bassPCA(x, y, n.pc = 20, save.yhat = F,
               n.cores = nc, verbose = F)
...
sens.func <- sobolBasis(mod, mcmc.use = 1, int.order = 2, verbose = F,
                          n.cores = nc)

## ----ex6plot3, fig.height=10*.7, fig.width=12*.7, out.width='\\linewidth', fig.cap='Variance decomposition as a function of space and time - pollutant spill model.'----
use <- which(apply(sens.func$S.var, 2, mean) > mean(sens.func$Var.tot) * .01)
par(mfrow=c(2, 3))
for(i in 1:length(s)) {
  ind <- length(t) * (i - 1) + 1:length(t)
  ...
  matplot(t, t(sens.func$S.var[1, use,ind]), type = 'l',
          main = paste('s =', s[i]), add = T, col = 2:5, lwd = 2)
}
